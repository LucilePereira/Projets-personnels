...
listemots=['ACTEUR', 'AVIRON', 'BOXEUR', 'BRONZE', 'BUDGET','CARTON', 'CHAQUE',
'CHEVAL','CIMENT', 'CLIENT','COMPTE', 'CONTRE', 'CUPIDE', 'DESIGN', 'DICTER',
'DOSAGE', 'DOUCHE','DROITE', 'EXPORT', 'FLAQUE', 'FORAGE', 'GLAIVE', 'GRAINE',
'GROUPE', 'JARDIN', 'JUNGLE','LOUCHE', 'LUCIDE', 'MANCHE', 'MARQUE', 'MIRAGE',
'MOUCHE', 'NIVEAU', 'NOVICE', 'OISEAU','PAQUET', 'PILOTE', 'PLANTE', 'POTEAU',
'PROJET', 'PUBLIC', 'REGAIN', 'RYTHME', 'SATIRE','SENTIR', 'SIMPLE', 'SONGER',
'SOUPLE', 'SQUARE', 'TROUPE', 'VALEUR']
...
for i in range(8):
    ...
    down()
    for j in range(6):
        write(grille[i][j], font=("Arial", 30)) #écrire chaque lettre une par une
        up()
        forward(50)
        down()
...
while coups<9:  #On limite le nombre de coups à 8
    mot=textinput("Mot proposé","Entrer un mot de 6 lettres: ").upper()
    #On enregistre le mot proposé par le joueur
    proposition.append(mot) #On met la proposition dans la liste de propositions
    mot=list(mot)
    ...
    #Si le mot ne fait pas 6 lettres, on change toutes les lettres par des *
        ...
        down()
        pencolor('red')
        write("Le mot doit avoir 6 lettres", font=("Arial", 30, "bold"))
        #message d'erreur
...
down()
pencolor('purple')
write("La réponse était :", font=("Arial", 35, "bold"))
up()
goto(60,-250)
down()
